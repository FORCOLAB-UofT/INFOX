...
class ForkAnalyser:
    def __init__(self, project_name, author, fork_info, code_clone_crawler):
        ...
        self.result_path = current_app.config['LOCAL_DATA_PATH'] + \
            "/" + self.project_name + '/' + self.author + '/result.json'
    ...
    def work(self):
        ...
        tf_idf_dict = self.code_clone_crawler.calc_key_words_tfidf(
            word_extractor.get_counter(self.all_tokens))  # change to fork_name
        ...
        ProjectFork(
            ...
            last_committed_time=datetime.datetime.strptime(
                self.last_committed_time, "%Y-%m-%dT%H:%M:%SZ"),
            created_time=datetime.datetime.strptime(
                self.created_time, "%Y-%m-%dT%H:%M:%SZ"),
            file_list=self.file_list,
            key_words=word_extractor.get_top_words_list(self.all_tokens, 100),
            key_stemmed_words=word_extractor.get_top_words_list(
                self.all_stemmed_tokens, 100),
            key_words_counter_dict=dict(
                (x, y) for x, y in word_extractor.get_top_words_tuple(self.all_tokens, 100)),
            key_words_by_tfidf=[x[0] for x in sorted(
                tf_idf_dict.items(), key=lambda x: x[1], reverse=True)[:100]],
            key_words_tf_idf_dict=dict((x, y) for x, y in sorted(
                tf_idf_dict.items(), key=lambda x: x[1], reverse=True)[:100]),
        ).save()
    ...
    def file_analyse(self, file):
        ...
        added_code = " ".join(filter(lambda x: (x) and (
            x[0] == '+'), changed_code.splitlines()))
        ...
        common_stemmed_tokens = word_extractor.get_top_words_list(
            stemmed_tokens, 100)
...
def update_progress(project_name, analyser_progress):
    Project.objects(project_name=project_name).update(
        analyser_progress=analyser_progress)
