...
class ForkUpdater:
    def __init__(self, project_name, author, fork_info, code_clone_crawler):
        ...
        self.diff_result_path = (
            current_app.config["LOCAL_DATA_PATH"]
            + "/"
            + self.project_name
            + "/"
            + self.author
    ...
    def get_tf_idf(self, tokens, top_number, list_option=True):
        tf_idf_dict = self.code_clone_crawler.calc_key_words_tfidf(
            word_extractor.get_counter(tokens)
        )
        result = [
            (x, y)
            for x, y in sorted(tf_idf_dict.items(), key=lambda x: x[1], reverse=True)
        ][:top_number]
        if list_option:
    ...
    def work(self):
        ...
        last_update = ProjectFork.objects(
            full_name=self.project_name + "/" + self.fork_name
        ).first()

        if (
            (not current_app.config["FORCED_UPDATING"])
            and (last_update is not None)
            and (
                datetime.strptime(self.last_committed_time, "%Y-%m-%dT%H:%M:%SZ")
                == last_update.last_committed_time
            )
        ...
        ProjectFork(
            ...
            fork_name=self.fork_name,
            project_name=self.project_name,
            last_committed_time=datetime.strptime(
                self.last_committed_time, "%Y-%m-%dT%H:%M:%SZ"
            ),
        ...
        file_distinct = list(
            OrderedDict.fromkeys(
                [x["file_full_name"] for x in compare_result["file_list"]]
            )
        )
        ...
        self.all_lemmatize_tokens = list(
            filter(
                lambda x: x not in project_name_stop_words, self.all_lemmatize_tokens
            )
        )
        ...
        ProjectFork(
            ...
            total_changed_line_number=sum(
                [x["added_line"] for x in compare_result["file_list"]]
            ),
...
def start_update(project_name, repo_info, forks_info):
    Project.objects(project_name=project_name).update(
    ...
    for fork in forks_info:
        ...
        try:
            ForkUpdater(
                project_name, fork["owner"]["login"], fork, code_clone_crawler
            ).work()
        except Exception as inst:
            print(inst)
        finally:
            Project.objects(project_name=project_name).update(
                analyser_progress="%d%%" % (100 * forks_count / forks_number)
            )
            Project.objects(project_name=project_name).update(
                last_updated_time=datetime.utcnow()
            )
