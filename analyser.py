...
def main():
    ...
    print ('---------------------------------------')
    with open(main_path + '/repo_info.json') as f:
    ...
    forks_info = {}
    with open(main_path + '/forks.json') as f:
    ...
    for dir in dir_list:
        if os.path.isdir(main_path + '/' + dir):
            with open(main_path + '/' + dir + '/commits.json') as f:
                commits = json.load(f)
                try:
    ...
    for (author, last_committed_time) in forks:
        ...
        if(last_committed_time > created_time):
            ...
            out_result = "fork_author: %20s, last committed time : %20s, created time: %20s, changed line: %d\n" % (author, last_committed_time, created_time, compare_result)
            print(out_result)
            with open('sorted_result.txt', 'a') as f:
                f.write(out_result)
